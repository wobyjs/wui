#!/usr/bin/env node
/**
 * Test: Cross select then click bullet, number, checkbox - detailed check
 */
...
async function runTest() {
    ...
    ws.on('open', async () => {
        ...
        await new Promise(r => setTimeout(r, 800));
        html = await getHTML(ws);
        console.log('After checkbox:', html);
        ...
        const checkResult = await sendCommand(ws, 'Runtime.evaluate', {
            ...
            (function() {
                ...
                const checkboxWrappers = shadow.querySelectorAll('#woby-checkbox-wrapper');
                const classWrappers = shadow.querySelectorAll('.checklist-item-wrapper');

                return {
                    checkboxCount: checkboxes.length,
                    checkboxWrapperCount: checkboxWrappers.length,
                    classWrapperCount: classWrappers.length,
                    firstLiHTML: shadow.querySelector('[contenteditable] li')?.outerHTML || 'not found'
                };
            })()
            `,
            returnByValue: true
