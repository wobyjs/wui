...
export const NumberField = ({ className, class: cls, children, onChange, noFix, onKeyUp, reactive, noRotate = false, ...props }: JSX.InputHTMLAttributes<HTMLInputElement> &
{ children?: JSX.Child, reactive?: ObservableMaybe<boolean>, noFix?: ObservableMaybe<boolean>, noRotate?: ObservableMaybe<boolean> }): JSX.Element => {
    const inputRef = $<HTMLInputElement>()
    const { min, max, value, step = 1, } = props
    ...
    return <div class={["number-input inline-flex border-2 border-solid border-[#ddd] box-border [&_*]:box-border", className, cls]}>
        ...
        />
        <button class={[btn, "plus"]} onPointerDown={() => startContinuousUpdate(true)} onPointerUp={stopUpdate} onPointerLeave={stopUpdate} disabled={cantMax}>+</button>
    </div>
}